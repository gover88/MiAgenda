...
class ReminderWorker(c:Context,p:WorkerParameters):Worker(c,p){
 ...
 companion object{fun schedule(c:Context,id:Long,title:String,due:Long,minutes:Int){val delay=(due-System.currentTimeMillis()-TimeUnit.MINUTES.toMillis(minutes.toLong())).coerceAtLeast(0);WorkManager.getInstance(c).enqueueUniqueWork("task_"+id,ExistingWorkPolicy.REPLACE,OneTimeWorkRequestBuilder<ReminderWorker>().setInitialDelay(delay,TimeUnit.MILLISECONDS).setInputData(workDataOf("id" to id.toInt(),"title" to title)).build())}}
}